...
class DQNAgent:
    ...
    - state_dim must match env.observation_space.shape[0] (now 5)
    """
    ...
    def act(self, state):
        ...
        s = torch.from_numpy(np.array(state, dtype=np.float32)).unsqueeze(0).to(self.device)
        with torch.no_grad():
    ...
    def push(self, state, action, reward, next_state, done):
        """Store a transition. done is cast to float internally."""
        self.memory.append((
    ...
    def train(self):
        """
        One Double-DQN gradient step on a random mini-batch.
        Returns loss value, or None if the buffer isn't warm yet.
        """
        ...
        a  = torch.from_numpy(np.array(actions,     dtype=np.int64 )).unsqueeze(1).to(self.device)
        r  = torch.from_numpy(np.array(rewards,     dtype=np.float32)).unsqueeze(1).to(self.device)
        ns = torch.from_numpy(np.array(next_states, dtype=np.float32)).to(self.device)
        d  = torch.from_numpy(np.array(dones,       dtype=np.float32)).unsqueeze(1).to(self.device)

        # Q(s, a) — value of the action actually taken
        q_values = self.q_net(s).gather(1, a)

        # Double DQN target: online net picks action, target net scores it
        with torch.no_grad():
    ...
    def end_episode(self):
        ...
        Decays epsilon and syncs the target network on schedule.
        """
